...
`optLevel: "O0"` LLVM does not run mem2reg, so every local really does live in
memory. That is exactly what `clang -O0` does, it is correct, and it makes test
binaries slower than they were, because that is the harness's default. It is not
a problem and it is not worth fixing.
