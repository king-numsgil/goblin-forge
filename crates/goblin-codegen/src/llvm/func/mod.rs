...
//!
//! [`ownership`] has copying, destroying and the arrays that need both —
//! REWRITE-PLAN §4.3's operations, which are one subject and the one this whole
//! design exists for. Everything else is here: the function shell, places,
//! statements, rvalues, operators, interfaces and calls.
//!
...
use goblin_mir::{
    Abi, BinOp, BlockId, BlockKind, Callee, CastKind, Category, ClassId, Const, FloatTy, FuncRef,
    Function, InterfaceId, LocalId, Module, Operand, Place, Projection, Rvalue, Statement,
...
impl<'a, 'm> Emitter<'a, 'm> {
    ...
    // Copying, destroying and arrays live in `ownership.rs`: one subject —
    // REWRITE-PLAN §4.3's operations — and that file says so at more length
    // than a section marker here could.
    // -- interfaces ----------------------------------------------------------
