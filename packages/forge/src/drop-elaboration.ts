...
export function needsDrop(module: Module, local: LocalDecl): boolean {
    ...
    if (category !== "Owning" && category !== "Polymorphic") {
        return false;
    }
...
function merge(into: InitState, from: InitState): void {
    for (const local of from.maybe) {
        into.maybe.add(local);
    }
    for (const local of into.definitely) {
        if (!from.definitely.has(local)) {
            into.definitely.delete(local);
        }
    }
}
...
function applyTerminator(
    ...
    switch (terminator.kind) {
        ...
        case "Switch":
            ...
            break;
        case "Call":
            for (const arg of terminator.args) {
                applyOperand(arg, tracked, state);
            }
...
function applyOperand(
    ...
    if (operand.kind !== "Move") {
        return;
    }
    if (operand.value.projection.length !== 0) {
        return;
    }
...
function successors(terminator: Terminator): number[] {
    switch (terminator.kind) {
        ...
        case "Call": {
            ...
            if (terminator.unwind.kind === "Cleanup") {
                out.push(terminator.unwind.value);
            }
            return out;
        }
        default:
...
function maybeSetFlag(
    ...
    if (flag !== undefined) {
        out.push({kind: "SetDropFlag", flag, value});
    }
}
