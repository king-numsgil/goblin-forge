...
function cLayouts(): Map<string, CLayout> {
  ...
  if (executable === undefined) throw new Error(`the layout oracle was not built into ${BUILD}`);

  const run = spawnSync(executable, [], { encoding: "utf8" });
  if (run.status !== 0) throw new Error(`the layout oracle exited ${run.status}`);
...
function goblinLayouts(): Map<string, LayoutReport> {
  ...
  for (const report of reports) out.set(report.name, report);
  return out;
}
